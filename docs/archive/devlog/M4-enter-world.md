...
**Статус:** ✅ **завершена** — вход в мир, управление (камера, бег, прыжки), чат,
серверный трекинг движения и сохранение позиции. Всё проверено живым клиентом.
...
## Чат, движение, персистентность (вторая часть M4)

- **Движение:** `MSG_MOVE_*` несут packed guid + MovementInfo. Парсим позицию (x,y,z,o),
  храним в сессии. Рассылки нет — игрок один, некому. Позиция сохраняется в `characters`
  при логауте и при разрыве соединения (проверено: перезаход в ту же точку).
- **Чат:** `CMSG_MESSAGECHAT` → эхо `SMSG_MESSAGECHAT` отправителю. Сообщение шлём
  сырыми байтами (кириллица сохраняется), язык — `LANG_UNIVERSAL`, тип — `CHAT_MSG_SAY=1`
  (в CMSG и SMSG разные enum'ы типа!).

### Главная засада чата (4 итерации): язык = НАВЫК, а не спелл

Клиент **локально** блокирует `/say` («Вы не знаете этого языка»), не отправляя пакет,
если у игрока нет языка. Список языков клиент строит из языковых **навыков** (skill),
а не из спеллов. Что не сработало и почему:
1. `SMSG_INITIAL_SPELLS` со spell-id языка — нет (язык не из спеллов). Плюс в 3.3.5
   spell-id там **u32**, а не u16 (gtker показал формат старой версии).
2. Сработало: **языковой навык в `PLAYER_SKILL_INFO_1_1`** (= `UNIT_END 0x94 + 0x1E8 = 0x27C`)
   прямо в self-`UPDATE_OBJECT`. 3 поля на навык: `skillId|step`, `value|max`, `bonus`.
   Навык 98 = Общий, 109 = Орочий и т.д. (≠ spell-id!). Значения 300/300.
   Индекс взят из TrinityCore UpdateFields.h (3.3.5).
