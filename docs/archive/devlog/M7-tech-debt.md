...
**Багфикс: стойки воина «все активны одновременно» (#21-регресс).**
`AuraPersistenceService.ReapplyPersistedAsync` восстанавливал перманентные переключатели
(стойки/ауры паладина/аспекты), строя `ActiveAura` вручную и **не проставляя `Group`** (=0). После
входа смена стойки (`ApplyAsync` снимает прочие `Group == 1`) восстановленную не трогала → стойки
копились. Фикс: восстанавливать переключатели через `AuraService.ApplyAsync` с группой/формой из
каталога тоглов (`SpellCatalog.TryGetToggle`). Бонус — само-лечение накопившихся дублей в
`character_aura`: восстановление второй стойки той же группы снимает первую (и её строку БД).
Лечится при следующем входе в мир.
