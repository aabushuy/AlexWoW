...
| **SQA-7 (M12.7)** Автотест маппинга `FromTemplate` (тир-1): детерминир. + фикстура 1995 спеллов + golden | ✅ |
...
## Автотест (M12.7, тир-1 — без БД и без игры)

Spell QA разделён на два тира: **тир-1 автотест** (детерминированный парсинг `spell_template→SpellInfo`,
гоняется в `dotnet test`) и **тир-2 захват в игре** (живой пайплайн: реальное оружие, моды талантов,
гейты каста, хил-манекен — то, что парсер не достаёт). Тиры дополняют друг друга.

- **Тир-1a** `SpellCatalogFromTemplateTests` — 14 детерминированных кейсов по категориям эффектов
  (урон/хил/weapon/%оружия/инстант+КД/%маны/энергия/DoT/HoT/бафф+HP/charge/крафт/семейство).
- **Тир-1b** `SpellCatalogFixtureTests` — фикстура **1995 реальных классовых спеллов** из mangos
  (`fixtures/spell_template_known.json`, выгружена одноразовым dotnet-дампом по тренер-шаблонам
  11/21/…/111 + ДК 28471 + стартовые): инвариант-прогон (min≤max, индексы эффектов, weapon/крафт/реагенты)
  + **golden-дайджест** SHA256 по всему разбору (любой дрейф парсера/дампа → падение; обновлять осознанно).
- `FromTemplate` сделан `internal` + `InternalsVisibleTo(AlexWoW.WorldServer.Tests)`.
- **Тонкость:** «значение в [min;max]» при общем источнике (spell_template) — тавтология; ценность тир-1 —
  в инвариантах и golden (дрейф), а сравнение «вычислено vs эталон» имеет смысл только для живого пайплайна (тир-2).

## Проверка

Build 0/0 + тесты **36/36** (10 `SpellTestAnalyzer` + 14 маппинг + 3 фикстурных + 9 прежних). Игровой приём: войти админ-персонажем,
