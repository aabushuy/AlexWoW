...
- ✅ **Живой клиент** (2026-06-02): экран «Создать персонажа», создание/удаление работают,
  список обновляется на месте. Голый персонаж (без стартовых предметов) — ок для M3.

## Грабли

- **Коды ответа off-by-one.** Сначала слали `0x2E`/`0x46` — это `*_IN_PROGRESS`, а не success.
  Клиент висел на экране (операция «в процессе»), хотя в БД всё проходило. Правильно:
  `CHAR_CREATE_SUCCESS=0x2F`, `CHAR_DELETE_SUCCESS=0x47`. Внимательно с ResponseCodes!
