...
- Добавить vmap-треугольники (WMO) как препятствия → навмеш обходит стены.
- Сериализация тайлов навмеша + серверная загрузка/запрос (`DtNavMesh`/`DtNavMeshQuery`).
- Тайловый навмеш на всю карту; полная экстракция + деплой.
- Потребитель — ИИ-перемещение существ (M6).
