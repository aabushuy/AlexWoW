...
## M5.5 — maps (высота рельефа) 🟡 загрузчик готов, ждёт данных клиента

Решение (выбор пользователя): **переиспользовать C++ экстрактор CMaNGOS**, на .NET — только загрузчик.
...
### Workflow экстрактора (шаг пользователя)

1. Собрать/взять экстрактор карт CMaNGOS-WotLK (из `cmangos/mangos-wotlk`, опция сборки
   extractors; формат должен быть **CMaNGOS**, не TrinityCore — magic `MAPS`/`v1.4`).
2. Запустить в корне клиента 3.3.5a → появится каталог `maps/` (файлы `*.map`).
3. Скопировать `maps/*` на homeserver в `/data/docker/alexwow-maps/`.
4. Пересоздать контейнер world — при входе в лог пойдёт высота земли (сверить с фактической Z).

### Дальше

- Проверить высоты живым клиентом (лог дельты у разных точек) → закрыть M5.5 (maps).
- vmaps (LoS) и mmaps (навигация) — отдельные срезы (ближе к M6: бой/ИИ).
