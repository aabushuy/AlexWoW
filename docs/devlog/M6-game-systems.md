...
| M6.7 ИИ существ | ✅ ответный бой + преследование/leash/реген + авто-агро по фракции — проверено клиентом |
...
## M6.7 — ИИ существ (инкремент 2b: авто-агро по фракции + доворот — ✅ проверено клиентом)

Враждебное существо **само** замечает подошедшего игрока и нападает; в мили **доворачивается** к
страфящему игроку. Завершает M6.7 (полный цикл: агро → погоня → бой → leash → возврат+реген).

### Что сделано
- **Реакции фракций из `FactionTemplate.dbc`** (клиентский, в дампе мира нет): команда
  `tools/MapExtractor factiontemplate <dataDir> <out.sql>` (`FactionTemplateExtract`) читает WDBC
  (14 полей×u32=56 б: id, faction, masks, enemy[4], friend[4]) → таблица `mangos.faction_template`
  (841 строка). Клиентские данные в репо не кладём (как maps/vmaps).
- **Загрузчик** `World/FactionStore` (DI-синглтон): ленивая загрузка всех реакций один раз
  (`WorldDatabase.GetFactionTemplatesAsync`), `IsHostile(creatureFt, playerFt)` по правилам CMaNGOS:
  явный враг по `enemy[]` → да; явный друг по `friend[]` → нет; иначе `hostileMask & target.ourMask`.
  Таблицы нет/пуста → авто-агро просто выключено (graceful).
- **Скан агро** (`CombatHandlers.TickAggroScanAsync`, из тика по каждому игроку): по видимым существам —
  если живое, не в бою, в радиусе ~18 ярдов и `IsHostile` → входит в бой (рык + погоня, общий
  `EnterCreatureCombatAsync` с ответкой инкр.1). Фракция игрока — `DisplayData.FactionForRace`.
- **Доворот в мили** (`WorldState.FaceCreatureAsync` + `SMSG_MONSTER_MOVE` тип `FACING_TARGET`):
  при страфе игрока существо разворачивается к нему на месте (троттлинг 400 мс + порог угла 0.2 рад).

### Грабли
- **`FACING_TARGET` с `count=0` → телепорт в текстуры**: клиент 3.3.5 всегда читает ≥1 точку пути после
  счётчика; при `count=0` читал мусор как координату. Фикс — `count=1` с ТЕКУЩЕЙ позицией (нулевой отрезок).
- `faction_template` живёт в MySQL-томе (переживает деплои; **перезалить при пересоздании тома** — как
  ручные индексы, см. задачу M7).
- Player faction template по расе уже корректен (`FactionForRace`: Human=1…Draenei=1629) — оба конца
  реакции (существо и игрок) есть в одной DBC.

### Проверка
- ✅ Сборка чистая, тесты 12/12. Старт: `faction_template: загружено 841`.
- ✅ **Живой клиент**: подойти к враждебному (красный) мобу → **сам агрится** и бежит; мирные (цыплёнок,
  городские NPC) **не агрятся**; в мили моб **доворачивается** к страфящему игроку без рывков.
...
## M6.11 — Система аур + M6.12 — Боевые ресурсы (ярость/стойки) ✅ (2026-06-08, проверено клиентом)

Всплыло при тесте воина (M9): не копилась ярость, не работали стойки. Разбили на M6.11 (ауры) и
M6.12 (ресурсы+стойки); стойки = shapeshift-аура, поэтому зависят от M6.11.

**M6.11 — система аур** (`Handlers/Auras.cs`, `Protocol/AuraPackets.cs`, `World/ActiveAura.cs`):
- Наложение/снятие/истечение аур игрока; `SMSG_AURA_UPDATE` (0x496) себе и наблюдателям; слот = позиция
  в баф-баре. Аура-форма (ShapeshiftForm != 0) выставляет `UNIT_FIELD_BYTES_2` байт 3 → форма (стойка).
- Тик истечения в `WorldState.UpdateAsync`; сброс аур при выходе. Дев-команды `.buff <spell> [сек]` / `.unbuff`.
- **ГЛАВНАЯ ГРАБЛЯ (фриз клиента):** само-каст ауры ТРЕБУЕТ бит **AFLAG_SELF_CAST (0x8)** — при нём caster
  в пакет НЕ пишется. Без бита клиент ждёт `PackedGuid` кастера, читает несуществующие байты → рассинхрон
  потока → зависание. wow_messages зовёт бит «NOT_CASTER» с ОБРАТНЫМ смыслом — верить TC/CMaNGOS
  (`if (!(flags & SELF_CAST)) << caster`). Флаги: EFF_INDEX_0=0x1, SELF_CAST=0x8, POSITIVE=0x10,
  DURATION=0x20, NEGATIVE=0x80 (из TrinityCore SpellAuraDefines.h).
- **Грабля №2 (стойки без иконки):** стойки воина в WotLK НЕ показываются бафф-иконкой — только панелью
  стоек (форма). Это нормально. Систему аур валидировали реальным баффом: `.buff 6673` (Боевой клич) →
  иконка с таймером появляется ✅.

**M6.12 — боевые ресурсы** (`Handlers/CombatResources.cs`):
- **Ярость** копится от нанесённого/полученного мили-урона (формула CMaNGOS `RewardRage`, хранится ×10),
  распадается вне боя; **энергия** разбойника регенит до 100. Хуки в `CombatHandlers` + тик в `WorldState`.
- **Стойки воина** (`SpellHandlers`): каст Battle/Defensive/Berserker (2457/71/2458) → аура-форма 17/18/19
  через M6.11 → стойка активна (панель/набор кнопок). ✅ проверено клиентом.
- Опкоды: `SMSG_AURA_UPDATE` 0x496, `UNIT_FIELD_BYTES_2` 0x7A. Расход ресурса абилками — когда абилки появятся.
