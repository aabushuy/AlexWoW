...
|---|---|---|
| MOD_BLOCK_PERCENT (51) + урон при блоке (43) | Блок щитом (Shield Block, Воин) ✅; Священный щит (Holy Shield 48952, Пал) 🟡 | +30% блока — каркас `BlockBonus` (как у воина); 🟡 BLOCK.2: при блоке урон Светом ~274 по атакующему (aura 43 PROC_TRIGGER_DAMAGE) — реализовано, ждёт проверки. Todo: 8 зарядов (сейчас на всё время баффа) |
