...
| MOD_DAMAGE_PERCENT_TAKEN (87) | Глухая оборона/Shield Wall −60% (Воин) ✅; Divine Protection −50% (Пал), Pain Suppression −40%/Dispersion −90% (Жрец), Barkskin −20% (Друид), Icebound Fortitude −30% (DK), Shamanistic Rage −30% (Шам) | 🟡 все data-driven одним каркасом (aura 87, misc 127 = все школы; формула `dmg×max(0,1+pct/100)` в `ResolveIncomingMelee`). Парсер по фикстуре подтверждает корректный `−%`/маршрутизацию (498/22812/48792 ✓); 33206/47585/30823 — те же данные. Ждёт живой проверки по классам (стенд: каст КД → `.dummy attack`, урон падает на %). Применяется к мили-урону существ (спелл-урон существ не моделируется) |
